...
local function check_surface(surface)
    ...
    write_log(string.format("copper: %05d, iron: %05d, coal: %05d, oil: %03d, coal rocks: %03d", copper, iron, coal, oil, coal_rocks))
    if iron >= iron_min and copper >= copper_min and oil >= oil_min and coal >= coal_min and coal_rocks >= coal_rocks_min then
        write_csv(string.format("%07d,%05d,%05d,%05d,%03d,%03d", seed, copper, iron, coal, oil, coal_rocks))
        local name = string.format("map.s%07d-%05d-%05d-%05d-%03d-%03d.png", seed, copper, iron, coal, oil, coal_rocks)
        if player() then
...
local function init()
    ...
    if (map_gen_settings == false) then
        map_gen_settings = game.surfaces[1].map_gen_settings
    end

    file_csv = file_prefix .. ".csv"
    file_log = file_prefix .. ".log"
    
    write_log("initializing seedfinder")
    write_csv("seed,copper,iron,coal,oil,coal_rocks,rocks")
